...
from collab_agents_miniproject.srv import target_global_pos, target_global_posResponse, target_local_pos, target_local_posResponse 
...
class MoveToPos():
    ...
    """
    def __init__(self, ID, UAVtype="master", masterID=1):
        ...
        if UAVtype == "slave":
            ...
            rospy.init_node("uav_swarm_slave_" + str(ID) , anonymous=True)
        else:
            rospy.init_node("uav_swarm_master", anonymous=True)        
        ...
        sg = rospy.Service(self.ID + "/target_global_pos", target_global_pos, self.GotoGlobPos)
        sl = rospy.Service(self.ID + "/target_local_pos", target_local_pos, self.GotoLocPos)
        ...
        rospy.loginfo("The " + self.ID + " target servieces were successfully initiated")
    ...
    def GotoLocalPos(self, x, y, z, yaw):
        ...
        rospy.loginfo(self.ID + ", estimated distance : " + str(dist))
        ...
        while ((np.linalg.norm(np.array(self.local_pos[0:3]) - np.array([x, y, z])) > 0.1) or
            ...
            if (self.UAV_state.mode != "OFFBOARD" and
                ...
                self.set_mode(0, 'OFFBOARD')
                print(self.ID + " enabling offboard mode")
                last_request = rospy.Time.now()
            else:
    ...
    def GotoLocPos(self, req):
        ...
        rospy.loginfo(self.ID +  ", received local target")
    ...
    def GotoGlobPos(self, req):
        ...
        rospy.loginfo(self.ID + ", received global target")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("usage: target_pos_server.py ID DroneType")

    MTP = MoveToPos(int(sys.argv[1]), sys.argv[2])
